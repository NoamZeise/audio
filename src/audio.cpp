  ...
  AudioData(std::string filename) {
    ...
    if (sf_readf_float(file, this->data, info.frames) != info.frames) {
      ...
      throw std::runtime_error(
          "incorect num of sampels loaded from audio data at " + filename);
    }
    sf_close(file);

    this->filename = filename;
  }
  ~AudioData() { delete[] data; }
  float *data;
  SF_INFO info;
...
struct Instance {
  Instance(AudioData *audio, bool loop, float volume) {
    ...
    PaError err = Pa_OpenDefaultStream(
        &stream, 0, audio->info.channels, paFloat32, audio->info.samplerate,
        FRAMES_PER_BUFFER, instanceCallback, this);
    if (err != paNoError)
      throw std::runtime_error("failed to open default stream for file at " +
                               this->audio->filename);
  }
  ...
  void Play() {
    ...
    if (err != paNoError)
      throw std::runtime_error(
          "failed to start stream at " + audio->filename +
          " Pa Error: " + std::string(Pa_GetErrorText(err)));
  }
  ...
  void Pause() {
    ...
    if (err != paNoError)
      throw std::runtime_error(
          "failed to stop stream at " + audio->filename +
          " Pa Error: " + std::string(Pa_GetErrorText(err)));
  }

  bool Playing() { return Pa_IsStreamActive(stream); }
...
Manager::~Manager() {
  ...
  // if(err != paNoError) throw std::runtime_error("failed to terminate Port
  // Audio");
}

void Manager::RemovePlayed() {
...
// optional, can frontload loading to make Play() faster the first time on a
// file
void Manager::LoadAudioFile(std::string filename) {
  loaded[filename] = new AudioData(filename);
}

void Manager::Play(std::string filename, bool loop, float volume) {
...
void Manager::Resume(std::string filename) {
  for (size_t i = 0; i < activeAudio.size(); i++)
    if (activeAudio[i]->audio->filename == filename && activeAudio[i]->paused)
      activeAudio[i]->Play();
}

void Manager::Pause(std::string filename) {
  for (size_t i = 0; i < activeAudio.size(); i++)
    if (activeAudio[i]->audio->filename == filename && !activeAudio[i]->paused)
      activeAudio[i]->Pause();
}

void Manager::Stop(std::string filename) {
  for (size_t i = 0; i < activeAudio.size(); i++)
...
void Manager::SetVolume(std::string filename, float volume) {
  for (size_t i = 0; i < activeAudio.size(); i++)
    if (activeAudio[i]->audio->filename == filename)
      activeAudio[i]->volume = volume;
}

bool Manager::Playing(std::string filename) {
  for (size_t i = 0; i < activeAudio.size(); i++)
    if (activeAudio[i]->audio->filename == filename &&
        activeAudio[i]->Playing())
      return true;
  return false;
}
